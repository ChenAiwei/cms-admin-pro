...
router.beforeEach(async(to, from, next) => {
    ...
    // 设置页面title
    //  document.title = to.meta.title == undefined ? siteTitle : to.meta.title + ' -' + siteTitle
    ...
    if (isLogin) { // 判断是否有token
        if (to.path === '/login') {
            ...
            if (hasRoles) {
                ...
                if (!roles.includes(to.meta.roleId) && to.name == null) {
                    if (from.path === '/') {
                        next({ name: indexPage })
                    }
                    ViewUI.LoadingBar.finish()
                ...
                try {
                    // 获取用户信息 这里可以进行一些操作
                    const { menuLevel } = await store.dispatch('user/getInfo')
                        // 根据角色生成可访问的路由
                    const accessRoutes = await store.dispatch('permission/generateRoutes', menuLevel)
                        // 增加异步路由
